...
use CLGTextTools::Observations::ContextVectorsFamily;
...
sub new {
	...
	# print("In ObsCollection line 72: ", @{$obsTypes});
...
sub addObsType {
    ...
    if (!defined($self->{families}->{$familyId})) {
		...
		} elsif ($familyId eq "CONTEXT") {
			$self->{families}->{$familyId} = CLGTextTools::Observations::ContextVectorsFamily->new({logging => defined($self->{logger})});
		} else {
...
sub extractObsFromText {
    ...
    foreach my $family (keys %{$self->{families}}) {
		...
		if ( ($family eq "WORD") || ($family eq "CHAR") || ($family eq "VOCABCLASS") || ($family eq "CONTEXT")) {
